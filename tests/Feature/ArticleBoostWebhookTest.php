...
test('boost webhook activates article boost status', function () {
    $author = Author::factory()->create();
    $article = Article::factory()->create(['author_id' => $author->id, 'boosted_at' => null]);
    ...
    $article->refresh();
    expect($article->boosted_at)->not->toBeNull();
    expect($article->boost_transaction_id)->toBe($paymentIntentId);
    ...
    $eventPayload = [
        ...
        'data' => [
            'object' => [
                'id' => $paymentIntentId,
                'object' => 'payment_intent',
                'metadata' => ['type' => 'boost', 'article_id' => $article->id],
            ],
        ],
    ];
...
test('boost webhook handles missing article gracefully', function () {
    $author = Author::factory()->create();
...
test('stripe service exception does not boost article', function () {
    $author = Author::factory()->create();
    $article = Article::factory()->create(['author_id' => $author->id, 'boosted_at' => null]);
    ...
    $article->refresh();
    expect($article->boosted_at)->toBeNull();
});
